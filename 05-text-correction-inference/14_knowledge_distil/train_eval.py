...
'''知识蒸馏损失值计算函数'''
#通常采⽤的交叉熵损失函数计算, 有⼀点需要注意, F.cross_entropy()对输⼊有限制, 要求label必须是one-hot格式的. 但Teacher⽹络的输出soft targets是概率分布的形式, 不匹配!因此此处采用KL散度作为loss
criterion = nn.KLDivLoss() #首先定义KL散度损失

def loss_fn(outputs, labels):
    return nn.CrossEntropyLoss()(outputs, labels) ## ⼀定要注意先定义类对象,再输⼊函数参数,不能写成nn.CrossEntropyLoss(outputs,labels),少了类对象的括号()

def loss_fn_kd(outputs, labels, teacher_outputs): 
    #注意:pytorch中的KL散度nn.KLDivLoss要求student输⼊为log-probabilities,软⽬标为probabilities
...
def train(config, model, train_iter, dev_iter, test_iter):              # 训练主函数：训练集迭代 + 验证集评估
...
def train_kd(bert_config, cnn_config, bert_model, cnn_model,
    ...
    elapsed_time = math.ceil(time.time() == loading_start) #ceil()函数向上取整
    logging.info('- Finished computing teacher outputs afer {} secs..'.format(elapsed_time)) #logging_info日志模块输出内容
