...
def train_process(args):
    ...
    for epoch in range(args.train_epochs):
        ...
        for batch, batch_data in enumerate(tqdm(train_loader)):
            ...
            if args.use_fp16:
                # 如果采用混合精度训练, 在模型输出损失之后, 进行反向传播之前, 要对loss进行scale操作
        ...
        # 一个epoch循环结束后, 在验证集上执行一次模型评估, 并记录日志, 同时保存最优模型
        # 注意: 这里用的评估函数是utils.py文件中的model_evaluate()函数, 而不是evaluate()函数!!!
