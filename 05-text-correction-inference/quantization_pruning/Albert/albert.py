# Albert模型：< A Lite BERT For Self-Supervised Learning Of Language Representations
    ...
    #相较于Bert的优化点：出发点是希望降低预训练的难度, 同时提升模型关键能⼒
        #词嵌入参数的因式分解：
            #通过中间映射层缩小词嵌入参数维度，使得模型整体参数更小
        #隐藏层之间的参数共享
            #bert中每一个encoder block的参数都独立，随着训练会build up，但是albert中，每一层参数都共享，本质只有一套encoder block，所以参数会更小，训练速度更快
        #去掉NSP，增加SOP预训练任务(Sentence Order Prediction)
            #NSP任务太简单，
        #去掉dropout操作：通过训练发现albert不容易过拟合，去掉dropout反而能提升模型性能
        #MLM任务的优化
            #segments-pair的优化：和BERT相反，AlBERT在90%的steps中使⽤了⻓度为512个token的⻓句⼦, 更⻓的句⼦可以提供更多上下⽂信息, 可以显著提升模型的能⼒.
            #Masked-Ngram-LM的优化：AlBERT预测的是Ngram⽚段, 每个⽚段⻓度为n (n=1,2,3), 每个Ngram⽚段的概率按照公式分别计算即可. ⽐如1-gram, 2-gram, 3-gram的概率分别为6/11, 3/11, 2/11.
    #模型代码：以下皆为模型构建部分
'''config代码'''
import torch
...
class Config(object):
    def __init__(self, dataset):
        ...
        self.save_path = './albert_saved_dict'
        self.save_path2 = './albert_saved_dict_quantized'
        ...
        self.albert_path = './albert_chinese_base'
        print(type(self.albert_path), self.albert_path)
        self.tokenizer = AlbertTokenizer.from_pretrained(self.albert_path) #分词/字器
        self.albert_config = AlbertConfig.from_pretrained(self.albert_path + '/config.json') #模型参数文件调取
...
class Model(nn.Module):
    def __init__(self,config):
        ...
        self.albert = AlbertModel.from_pretrained(config.albert_path,config=config.albert_config) #模型基座
    ...
    def forward(self, x):
        ...
        out = self.albert(context, attention_mask=mask) #context为数字化的文本，attention_mask为注意力掩码
...
if __name__ == '__main__': #主函数
    dataset = 'toutiao' #数据集
    if args.model =='albert': #模型选择

        model_name = 'albert' #模型名称
    ...
    #调用:所在目录终端输入：python albert.py --model albert