...
def load_model_and_parallel(model, gpu_ids, ckpt_path=None, strict=True):
    # 当服务器只有单⼀GPU时, 可以写下⾯⼀⾏代码
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # 如果服务器有多块GPU时, 可以指定某⼀块GPU作为主设备
    device = torch.device('cpu' if torch.cuda.is_available() else 'cuda:' + gpu_ids[0])
...
class Data_Processor():
    ...
    def get_label_data(self):
        ...
        res = []
        # 遍历数据集, 每条样本包含2部分, 原始⽂本text, 命名实体的字符串stock_name
        for item in data:
