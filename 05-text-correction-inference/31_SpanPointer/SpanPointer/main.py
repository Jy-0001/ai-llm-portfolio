...
# 训练基础函数, 真正的训练流程在最后⼀⾏的train()函数中
def train_base(config, train_feature, dev_feature=None, test_feature=None):
    # 加载实体映射表
    with open(
        os.path.join(config.raw_data_dir, f'{config.task_type}_ent2id.json'),
        encoding='utf-8'
    ) as f:
        ent2id = json.load(f)
...
def training(config):
    ...
    # 调⽤processor的内部函数处理后, train数据格式:
    # {'text': text, 'stock_name': [(stock_name, start_pos, end_pos), ...]}
    train_feature, dev_feature = processor.get_label_data()

    # 调⽤train_base为主训练函数
    train_base(config, train_feature, dev_feature)
