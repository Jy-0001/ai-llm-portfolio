#基于bert预训练模型的数据处理：以投满分项目为例
    #数据预处理：
        #第⼀步: 查看项⽬数据集(包括标签集、训练集、验证集、测试集)
            #标签文件
'''文件目录bash输入:head -n 5 bert_classification.txt'''
            #训练数据集: train.txt中包含180000⾏样本, 每⾏包括两列, 第⼀列为待分类的中⽂⽂本, 第⼆列是数字化标签, 中间⽤\t作为分隔符.
'''文件目录bash输入:head -n 5 train_dataset.txt'''
            #验证数据集: dev.txt中包含10000⾏样本, 每⾏包括两列, 第⼀列为待分类的中⽂⽂本, 第⼆列是数字化标签, 中间⽤\t作为分隔符.
'''文件目录bash输入:head -n 5 dev_dataset.txt'''
            #测试数据集: test.txt中包含10000⾏样本, 每⾏包括两列, 第⼀列为待分类的中⽂⽂本, 第⼆列是数字化标签, 中间⽤\t作为分隔符.
'''文件目录bash输入:head -n 5 test_dataset.txt'''
        #第⼆步: 查看预训练模型相关数据
            #预训练模型相关数据：
                #超参数配置文件config.json：其中包含了预训练模型的超参数设置，使用前先看
'''文件目录bash输入:head -n 5 config.json'''

                #预训练模型文件model.bin：模型本体文件，
'''模型文件.bin由下载获取'''
                #模型词典文件vocab.txt：包含词表vocab
'''文件目录bash输入:head -n 5 vocab.txt'''
        #第三步: 编写⼯具类函数（此段代码仅提供解释，实际代码在utils.py中）
