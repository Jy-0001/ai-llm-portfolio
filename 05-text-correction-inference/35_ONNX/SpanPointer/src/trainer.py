...
def build_optimizer_and_scheduler(config, model, t_total):
    ...
    num_warmup_steps=int(config.warmup_proportion * t_total), num_training_steps=t_total)
    return optimizer, scheduler

# 外层的主函数main.py -> train_base() -> train(), 这⾥才是整个训练的主函数逻辑代码
def train(config, model, train_dataset, dev_dataset, ent2id):
    # 本项⽬中采⽤Span Pointer模型进⾏NER任务实现
    ...
    # 进⾏⽇志写⼊, 追踪
    logger.info('batch_size: {}, epochs:{}, train_nums: {}, dev_nums: {}'.format(config.train_batch_size, config.train_epochs, len(train_dataset), len(dev_dataset)))
    ...
    for epoch in range(config.train_epochs):
        # 每⼀个epoch开始前清空CUDA缓存, 节省GPU资源, 在CPU环境下训练的同学可以注释掉下⾯⼀⾏代码
        ...
        for step, batch_data in enumerate(tqdm(train_loader)):
            ...
            model.zero_grad()
            avg_loss += loss.item()
        # 当⼀轮epoch训练结束后, 在验证集上进⾏⼀次评估
        f1 = model_evaluate(model, dev_loader, config, device, ent2id)
        # 直接调⽤计算关键字指标的函数, 函数代码位于conlleval.py中
        f1 = calculate(config)
        if f1 > best_f1:
            best_f1 = f1
