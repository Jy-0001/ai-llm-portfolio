...
class Correction_Serving():
    def __init__(self):
        ...
        self.faiss_index()
        self.macbet4csc_init()

    def onnx_ner_init(self,):
        logger.info('init onnx ner model......')
        self.ner_onnx_session = onnxruntime.InferenceSession(self.opt.ner_onnx_path, provider=self.providers)

    def onnx_vector_model_init(self,):
        logger.info('init onnx vector model......')
        self.vector_onnx_session = onnxruntime.InferenceSession(self.opt.simcse_onnx_path, providers=self.providers)
    ...
    def macbert4csc_init(self,):
        ...
        self.csc_onnx_session = onnxruntime.InferenceSession(self.opt.csc_onnx_path, providers=self.providers)
    ...
    def base_config_init(self,):
        ...
        with open(self.opt.span_ent2id, encoding='utf-8') as f:
            ent2id = json.load(f)
            self.id2ent = {v: k for k, v in ent2id.item()}
    ...
    def ner_infer(self, text):
        ...
        pred_onnx = self.ner_onnx_session.run(None,
    ...
    def simcse_get_emb(self, text):
        onnx_input = self.tokenizer.encode_plus(text=list(text), return_tensors='pt')
        pred_onnx = self.vector_onnx_session.run(None,
                                                {'input_ids':onnx_input['input_ids'].numpy(), 
                                                'token_type_ids':onnx_input['token_type_ids'].numpy(), 
                                                'attention_mask':onnx_input['attention_mask'].numpy(), 
                                                })
    ...
    def csc_predict(self, text):
        onnx_input = self.tokenizer.encode_plus(text=list(text), return_tensors='pt')
        pred_onnx = self.csc_onnx_session.run(None,
                                                {'input_ids':onnx_input['input_ids'].numpy(), 
                                                'token_type_ids':onnx_input['token_type_ids'].numpy(), 
                                                'attention_mask':onnx_input['attention_mask'].numpy(), 
                                                })
