...
class BertWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        # 纠错输出
        cor_out = bert_outputs.logits
        # 检测输出
        det_out = self.sigmoid(self.detection(bert_outputs.hidden_states[-1])).squeeze(-1)
        return det_out, cor_out
...
class OnnxTrans():
    ...
    def init_model(self):
        ...
        self.model.load_state_dict(torch.load('./output/bert4csc/best_model.pt', map_location='cpu'), strict=True)
        self.model.eval()
    ...
    def init_onnx(self):
        ...
        # 用包装模型替换原始模型
        self.model = BertWrapper(self.model)
        self.model.eval()
        onnx_input = self.make_onnx_infer_input()

        # 明确模型的输入, 输出, 动态轴的设置
        dynamic_axes = {'input_ids': [1], 'attention_mask': [1], 'token_type_ids':[1], 'output1': [1], 'output2': [1]}
