...
class DataCollator:
    def __init__(self, tokenizer):
        # 数据处理类中的tokenizer, 采用参数模式传入
        self.tokenizer = tokenizer

    def __call__(self, data):
        # 提取源文件数据, 共3部分
        # original_texts: 原始待纠错的文本
        # correct_texts: 正确的文本
...
class CscDataset(Dataset):
    ...
    def __getitem__(self, index):
        # 返回原始数据文件中的JSON信息, 3个key值为文件中的原值, 必须保持一致
