...
class Supervised(data.Dataset):
    def __init__(self, path='./data/'):
        ...
        self.data_dir = os.path.join(path, 'train_medical.json')
        self.train_data = []
    ...
    def _create_train_data(self):
        if str(self.data_dir).endswith('jsonl'):
            data = []
        ...
        for item in data:
            # 以⼆元组的形式添加进训练集列表中
            self.train_data.append((item['source'], item['target']))
    ...
    #------------------------------------------------------以下代码或针对错误的数据
    # def __getitem__(self, index):
    #     text, pos_text, neg_text= self.train_data[index]
    #     # 调⽤类内函数, 对三元组进⾏tokenizer处理

    #     return self.tokenizer_process(text, pos_text, neg_text)
    #------------------------------------------------------以下代码或针对错误的数据
...
class Infer():
    ...
    def get_companys(self):
        company_list = []
        # with open(file='./data/股票名称.csv', mode='r', encoding='utf-8') as f:
        # with open(file='./data/医疗术语.csv', mode='r', encoding='utf-8') as f:
        with open(file='./data/query_dict.txt', mode='r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                if i == 0:
                    continue
                line = [x for x in line.strip().split('\t') if x != '不详' and x != 'None' and x]
