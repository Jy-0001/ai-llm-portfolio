...
'''加载训练集, 测试集的原始数据'''
def load_file(file_path): #返回数据集中的一句话，用于加载到模型中。
    ...
    for line in contents:
        if line != '\n':
            ...
            texts.append(text) # 此处texts为整个数据集文件中所有token被分为句序列的合集
            labels.append(label) # 此处labels为整个数据集文件中所有标签被分为句序列的合集
            text = []
            label = []
    return texts, labels
...
def load_data(file_path, max_length, label_dic, vocab):
    ...
    for i in range(len(texts)):
        ...
        token = texts[i] # 此处token为一个列表，储存一句话中的所有token
        label = labels[i]
        '''数据裁剪'''
        ...
        '''数字化张量映射'''
        input_ids = [int(vocab[i]) if i in vocab else int(vocab['[UNK]']) for i in token_f]
        ...
        '''实例化特征对象'''
        feature = InputFeatures(text=token_f, label=label_f, input_id=input_ids, input_mask=input_mask, label_id=label_ids)
