...
from transformers import BertModel, BertTokenizer, BertConfig #修改为bert
...
class IDCNN_CRF(nn.Module):
    def __init__(self, vocab_size, tagset_size, embedding_dim, num_filters=64, dropout=0.4, use_cuda=True):
        ...
        self.bert_path = './bertmodel'#-----------------------------bert修改----------------------------------
        self.tokenizer = BertTokenizer.from_pretrained(self.bert_path) #分词/字器（固定用bert分词）#-----------------------------bert修改----------------------------------
        self.bert_config = BertConfig.from_pretrained(self.bert_path + '/config.json') #模型参数文件调取#-----------------------------bert修改----------------------------------


        # 1: 设置词嵌入层, 这是所有NLP模型必备的第一步
        # self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.bert = BertModel.from_pretrained(self.bert_path) #-----------------------------bert修改----------------------------------
        # 2: 实例化IDCNN类, 输入维度是嵌入层的输出维度
        # self.idcnn = IDCNN(input_size=embedding_dim, filters=num_filters)
        self.idcnn = IDCNN(input_size=self.bert.config.hidden_size, filters=num_filters)
    ...
    def get_output_score(self, sentence, attention_mask): #-----------------------------bert修改----------------------------------
        ...
        bert_out = self.bert(input_ids=sentence, attention_mask=attention_mask).last_hidden_state #-----------------------------bert修改----------------------------------

        # 3: 执行初始化函数的第3层
        # idcnn_out = self.dropout(idcnn_out)
        idcnn_out = self.idcnn(bert_out, seq_length)
        idcnn_out = self.dropout(idcnn_out) #-----------------------------bert修改----------------------------------
        
        # 4: 执行初始化函数的第5层
        # out = self.linear(idcnn_out)
        out = self.linear(idcnn_out) #-----------------------------bert修改----------------------------------
    ...
    def forward(self, sentence, masks):
        # 首先获取发射矩阵的输出张量
        feats = self.get_output_score(sentence,masks)
    ...
    def neg_log_likelihood_loss(self, sentence, mask, tags):
        ...
        feats = self.get_output_score(sentence, mask) #-----------------------------bert修改----------------------------------
