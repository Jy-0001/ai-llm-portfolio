...
class QuestionClassifier:
    def __init__(self):
        ...
        self.disease_path = os.path.join(cur_dir, 'dict/disease.txt')          # disease_path:str 疾病词表文件路径
        self.drug_path    = os.path.join(cur_dir, 'dict/drug.txt')             # drug_path:str 药品词表文件路径
        self.food_path    = os.path.join(cur_dir, 'dict/food.txt')             # food_path:str 食物词表文件路径
        self.symptom_path = os.path.join(cur_dir, 'dict/symptom.txt')          # symptom_path:str 症状词表文件路径

        self.disease_words = [i.strip() for i in open(self.disease_path) if i.strip()]  # disease_words:list[str] 逐行读入疾病词
        self.drug_words    = [i.strip() for i in open(self.drug_path) if i.strip()]     # drug_words:list[str] 逐行读入药品词
        self.food_words    = [i.strip() for i in open(self.food_path) if i.strip()]     # food_words:list[str] 逐行读入食物词
        self.symptom_words = [i.strip() for i in open(self.symptom_path) if i.strip()]  # symptom_words:list[str] 逐行读入症状词

        self.region_words = set(self.disease_words + self.drug_words + self.food_words + self.symptom_words)  # region_words:set[str] 四类实体词合并去重
    ...
    def classify(self, question):
        ...
        if self.check_words(self.symptom_request, question) and ('disease' in types):  # bool & bool：触发词命中 + 句子里有 disease 类型
            question_types.append('disease_symptom')                            # list[str] += 1：问“某疾病的症状”

        if self.check_words(self.food_request, question) and ('disease' in types):     # 同理：饮食触发词命中 + 有 disease
            question_types.append('disease_food')                               # list[str] += 1：问“某疾病吃什么/忌口”

        if self.check_words(self.drug_request, question) and ('disease' in types):     # 同理：药品触发词命中 + 有 disease
            question_types.append('disease_drug')                               # list[str] += 1：问“某疾病用什么药”

        if question_types == [] and 'symptom' in types:                         # 兜底：没有触发词分类，但识别到了 symptom 实体
