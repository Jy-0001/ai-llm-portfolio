...
#bert模型搭建
    #第一步：编写两个模型类的代码
        #实现config类代码:管“环境与超参数”（不是模型本体），它负责存放/组织所有训练需要的配置，如训练路径，训练超参，设备等
'''config代码'''
import torch
...
class Config(object):
    def __init__(self, dataset):
        ...
        self.train_path = self.data_path + 'class11_bert_train.txt' #训练集
        self.dev_path = self.data_path + 'class11_bert_dev.txt' #验证集
        self.test_path = self.data_path + 'class11_bert_test.txt' #测试集
        self.class_list = [x.strip() for x in open(self.data_path + 'class11_bert_classification.txt').readlines()]
        self.save_path = './class11_bert_saved_dict'
        ...
        self.num_epochs = 2 #epoch数，一般<=3
        # self.batch_size = 128 #mini-batch大小
        self.batch_size = 32 #mini-batch大小
        self.pad_size = 32 #每句话处理成的长度
        self.learning_rate = 5e-5 #学习率
        self.bert_path = './bertmodel'
        self.tokenizer = BertTokenizer.from_pretrained(self.bert_path) #分词/字器（固定用bert分词）
        self.bert_config = BertConfig.from_pretrained(self.bert_path + '/config.json') #模型参数文件调取
...
class Model(nn.Module):
    def __init__(self,config):
        ...
        self.bert = BertModel.from_pretrained(config.bert_path,config=config.bert_config) #模型基座
    ...
    def forward(self, x):
        ...
        out = self.bert(context, attention_mask=mask) #context为数字化的文本，attention_mask为注意力掩码
...
from class11utils import get_time_dif
    ...
    for epoch in range(config.num_epochs):  #训练epoch数
        ...
        for i, (trains, labels) in enumerate(train_iter): #训练一个epoch
            ...
            if total_batch % 200 == 0 and total_batch != 0:
                # 每200轮输出在训练集和验证集上的效果
                true = labels.data.cpu() #转到cpu上
                predic = torch.max(outputs.data, 1)[1].cpu() #获取最大概率的标签
